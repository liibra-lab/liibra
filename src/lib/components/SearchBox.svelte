<script lang="ts">
	...
	let { m, value = '', variant = 'compact' }: { m: Messages; value?: string; variant?: 'compact' | 'hero' } =
		$props();
</script>

<form action="/search" method="GET" role="search" class="flex w-full gap-2">
	<label class="sr-only" for="q">{m.search_label}</label>
	<input
		id="q"
		name="q"
		type="search"
