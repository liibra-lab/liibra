...
| `npm run test:e2e` | Playwright smoke tests against the built Worker in `wrangler dev` (run `npm run build` first). |
| `npm run check` | Generate Wrangler types, sync SvelteKit, and run `svelte-check`. |
...
- `worker-configuration.d.ts` is generated by `wrangler types` (via `npm run check` or `npm run build`) and is not committed.
...
e2e/                   Playwright smoke tests (real Workers runtime via wrangler dev)
